...
void sppctl_pin_set( sppctl_pdata_t *_p, u8 _pin, u8 _fun) {
 u32 *r;
 sppctl_reg_t x = {  .m = 0x007F, .v = ( u16)_pin  };
 u8 func = ( _fun >> 1) << 2;
 if ( _fun % 2 == 0) ;
 else {  x.v <<= 8;  x.m <<= 8;  }
...
u8 sppctl_fun_get( sppctl_pdata_t *_p, u8 _fun) {
 u8 pin = 0x00;
 u8 func = ( _fun >> 1) << 2;
 sppctl_reg_t *x;
 u32 r = readl( _p->moon_base + func);
 x = ( sppctl_reg_t *)&r;
...
static int sppctl_remove( struct platform_device *_pdev) {
 ...
 sppctl_sysfs_clea( _pdev);
 sppctl_pinctrl_clea( _pdev);
 kfree( p);
 return( 0);  }
